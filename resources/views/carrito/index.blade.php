            ...
            <div class="card" style="border: 1px solid #97ACBA;">
                <div class="card-header" style="background-color: #FFF9F0;">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 id="card_title" style="color: #404E5E; font-weight: 600;">
                            {{ __('Carrito de compras') }}
                        </h5>
                        <a href="{{ route('carritos.create') }}" class="btn btn-primary btn-sm" style="background-color: #5D8EC6; border-color: #5D8EC6;">
                            {{ __('+ Nuevo carrito') }}
                        </a>
                    </div>
                </div>
                                    ...
                                    <tr>
                                        <td style="color: #404E5E;">
                                            @if($carrito->cliente_id)
                                                @php 
                                                    $cliente = App\Models\Cliente::find($carrito->cliente_id); 
                                                @endphp
                                                @if($cliente)
                                        ...
                                        <td style="color: #404E5E;">
                                            @php
                                                // Aseguramos que 'productos' sea un arreglo
                                        ...
                                        <td>
                                            <form action="{{ route('carritos.destroy', $carrito->_id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <a class="btn btn-sm" style="background-color: #5D8EC6; color: #ffffff; border-color: #5D8EC6;"
                                                ...
                                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de eliminar este carrito?')">
